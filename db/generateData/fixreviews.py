...
for i in r.keys():
    for j in range(n):
        messages = [
            {"role": "system", "content": "You are a helpful assistant who generates short, realistic, and generic product reviews based on a given rating. Do not include any other text than the review, and do not include quotation marks in your response."},
        ]
        messages.append({"role": "user", "content": f"Generate a short, realistic, generic review for a product based on the following rating: {i} stars. It should be less than 20 words."})
...
reviews_df.to_csv("reviews.csv", index=False, quoting=csv.QUOTE_NONE)
